/**
 * @file tsumikoro_crc8.c
 * @brief CRC8-CCITT implementation for Tsumikoro bus protocol
 *
 * Copyright (c) 2025 Yann Ramin
 * SPDX-License-Identifier: Apache-2.0
 */

#include "tsumikoro_crc8.h"

/**
 * @brief CRC8-CCITT lookup table (256 bytes)
 *
 * Pre-computed for polynomial 0x07
 * Generated with: crc = (crc << 1) ^ ((crc & 0x80) ? 0x07 : 0)
 */
static const uint8_t crc8_table[256] = {
    
};

uint8_t tsumikoro_crc8_calculate(const uint8_t *data, size_t len)
{
    uint8_t crc = 0x00;  // Initial value

    if (data == NULL) {
        return 0;
    }

    for (size_t i = 0; i < len; i++) {
        crc ^= data[i];

        // Process each bit
        for (uint8_t bit = 0; bit < 8; bit++) {
            if (crc & 0x80) {
                crc = (crc << 1) ^ TSUMIKORO_CRC8_POLY;
            } else {
                crc = (crc << 1);
            }
        }
    }

    return crc;
}

uint8_t tsumikoro_crc8_calculate_table(const uint8_t *data, size_t len)
{
    uint8_t crc = 0x00;  // Initial value

    if (data == NULL) {
        return 0;
    }

    for (size_t i = 0; i < len; i++) {
        crc = crc8_table[crc ^ data[i]];
    }

    return crc;
}

int tsumikoro_crc8_verify(const uint8_t *data, size_t len, uint8_t expected_crc)
{
    uint8_t calculated_crc = tsumikoro_crc8_calculate_table(data, len);
    return (calculated_crc == expected_crc) ? 1 : 0;
}
