...
bool tsumikoro_hal_transmit(tsumikoro_hal_handle_t handle, const uint8_t *data, size_t len)
{
    ...
    if (device->tx_active) {
        return false;  // TX already in progress
    }
...
size_t tsumikoro_hal_receive(tsumikoro_hal_handle_t handle, uint8_t *buffer, size_t buffer_len)
{
    ...
    while (count < buffer_len && device->rx_count > 0) {
    ...
    if (count < buffer_len) {
        ...
        if (len > 0) {
            ...
            size_t to_copy = (len < (buffer_len - count)) ? len : (buffer_len - count);
